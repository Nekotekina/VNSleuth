...
int main(int argc, char* argv[])
{
	...
	if (argc == 2) {
		...
		std::cerr << "Usage 0: " << argv[0] << " <game_directory>" << std::endl;
		...
		std::cerr << "Usage 2: " << argv[0] << " <game_directory> -m <model> [<LLAMA args>...]" << std::endl;
	...
	if (g_mode == op_mode::print_info) {
		...
		if (argc > 3 && argv[3] == "--repair"sv) {
			...
			for (auto& seg : g_lines.segs) {
				if (!seg.lines.at(0).tr_text.empty())
					update_segment(&seg - g_lines.segs.data());
			}
		}
	...
	if (argc > 2) {
		argc--;
		argv++;
		if (g_mode == op_mode::rt_cached || g_mode == op_mode::print_only) {
			argc--;
			argv++;
		}
	...
	while (!g_stop && std::getline(std::cin, line)) {
		...
		if (g_history.empty() && next_id.second) {
			next_id.second = 0;
		...
		while (next_id <= last_id) {
			...
			if (std::unique_lock lock(g_mutex); should_append_history()) {
				...
				if (next_id.second == 0 && !g_history.empty()) {
					...
					if (!translate(params, {0, 0}, tr_cmd::sync))
						return 1;
					if (!update_segment(next_id.first))
						return 1;
			...
			} else if (g_mode == op_mode::rt_llama && g_history.back().first != next_id.first) {
				lock.unlock();
				...
				if (prev_id == c_bad_id) {
					std::cerr << "Unexpected jump, terminating. Try to restart." << std::endl;
					g_stop = true;
					break;
				}
